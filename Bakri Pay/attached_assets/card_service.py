...
class CardService:
    ...
    def get_user_cards(user_id, account_id=None):
        ...
        """
        try:
            query = Card.query.join(Account).filter(Account.user_id == user_id)
    ...
    def toggle_card_status(user_id, card_id, activate=True):
        ...
        try:
            ...
            if not card:
                raise NotFound(description="Card not found")
    ...
    def _generate_card_number():
        ...
        # Start with SadaPay's IIN (Issuer Identification Number)
        prefix = '4242'
    ...
    def _hash_cvv(cvv):
        ...
        return f"hashed_{cvv}"  # Replace with actual hashing
